...
export const adminCreateUserSchema = z.object({
  ...
  kycStatus: z.enum(['pending', 'submitted', 'verified', 'rejected', 'not required']).optional().default('pending'),
  password: passwordSchema,
...
export const adminUpdateUserSchema = z.object({
  ...
  panNumber: z.string().trim().regex(/^[A-Z]{5}[0-9]{4}[A-Z]{1}$/i, 'must be a valid 10-character PAN (e.g. ABCDE1234F)').optional().or(z.literal('')),
  passportPhoto: mediaUrl,
  kycStatus: z.enum(['pending', 'submitted', 'verified', 'rejected', 'not required']).optional(),
  avatar: mediaUrl,
